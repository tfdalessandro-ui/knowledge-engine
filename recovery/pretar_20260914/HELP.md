...
**What it takes to unblock the 4 remaining connectors:** an Azure AD app
registration in a real Microsoft 365 tenant (Entra admin center ->
App registrations -> New registration), with delegated or application
permissions granted for `Sites.Read.All` (SharePoint/OneDrive),
`Mail.Read` (Outlook), and `ChannelMessage.Read.All`/`Chat.Read`
(Teams) -- none of which this project has access to, since it needs a
real organization's tenant, not something that can be self-provisioned
from a dev machine. Once a client ID/secret (or certificate) and tenant ID
exist, they'd be read the same way every other credential in this repo is
-- a `KE_MSGRAPH_*` env var / `.env` entry, never hardcoded -- and each
connector implemented against the Microsoft Graph SDK, following the same
"tag every ingested document with its access grants at ingest time"
pattern the Git connector already establishes in `src/access/`. Until
then, `NOT_CONFIGURED` is the accurate status, not a bug to silently work
around.
...
## Running the research-to-production loop (Build Kickoff Step 2)

**A SEPARATE crawl target from P6's above -- do not confuse the two.**
`research/allowlist.py`'s `RESEARCH_ALLOWLIST` (arXiv cs.IR/cs.NE/cs.AI
category listings) feeds candidate-technique discovery only; fetched
pages never enter the searchable corpus/index.

```bash
PYTHONPATH=src .venv/bin/python -m research.pipeline   # crawl research sources
```

Writes to `data/research_papers/` / `data/research_crawl_state.db` --
distinct paths from P6's `data/crawled/` / `data/crawl_state.db`.
Respects arXiv's own `Crawl-delay: 15` (checked directly against
arxiv.org's robots.txt, not assumed), a different rate than P6's default
3.0s.

**Registering a candidate technique** (a real engineering step, done once
per candidate -- reading a paper/idea and turning it into an actual code
change is not automated):

```python
from research.candidates import CandidateStore
from config import get_settings

store = CandidateStore(get_settings().research_candidates_db_path)
store.register(
    name="short_slug",
    source="paper title/URL or where the technique idea came from",
    description="what config/code actually changed",
    config_diff='{"module.CONST": {"before": X, "after": Y}}',
    benchmark_before=<P0 harness nDCG@10 before>,
    benchmark_after=<P0 harness nDCG@10 after>,
)
```

`decision_date` is set automatically to 14 days out
(`research.candidates.DECISION_DELAY_DAYS`).

**The mechanical decision + rollback check:**

```bash
PYTHONPATH=src .venv/bin/python scripts/research_loop.py
```

Two rules, both in `scripts/research_loop.py`'s `decide()` and
`check_rollback()` functions (small enough to read directly rather than
take on faith):
1. Any `pending` candidate whose `decision_date` has passed gets a
   mechanical decision: `benchmark_after >= benchmark_before` ->
   `promoted`; otherwise -> `dropped`. No partial credit, no built-in
   human override (a human can always edit the `CandidateStore` directly).
2. After any promotion, if the live version's score is below the average
   of the trailing 5 recorded version scores
   (`research.candidates.ROLLING_WINDOW_SIZE`), the best of those 5
   should be restored (the check reports this; actually reverting the
   config is a manual step from `version_history`, not applied
   automatically by this script).

Writes a full `LOGBOOK_*.md` entry every time it makes a real decision or
triggers the rollback check positively -- prints a one-line "nothing due"
message and skips the logbook entry otherwise, same skip-vs-log
discipline as `scripts/auto_tune.py`.

Both the research crawl and the decision/rollback check run on their own
systemd user timers, not cron (isolated from this node's unrelated
Sensalis/VCN cron jobs, same reasoning as the P6 crawl's `ke-crawl.timer`):
`ke-research-crawl.timer` (daily 04:00 UTC), `ke-research-loop.timer`
(daily 04:30 UTC, 30 minutes after the crawl to give it time to finish).
...
**A sample of 3 is small.** It's enough to demonstrate the misattribution
failure mode is real and to avoid the false confidence of "0 hallucinated
citations" standing in for "correct," but not enough to estimate how often
it happens at scale. Before relying on P7 citation accuracy for anything
beyond this demo corpus, run a larger manual side-by-side pass (more
sampled queries, ideally across different topics/corpus sections) and
report the resulting ratio the same honest way -- see
...
(e.g. sensalis-node) and real network access: **202 passed, 1 xfailed, 1
skipped** (the PDF test). The one `xfailed` is
`test_hybrid_vs_bm25.py::test_hybrid_beats_bm25_only_by_5_percent_ndcg10`
— expected, not a regression to chase: the P2 exit criterion is currently
genuinely not met at this corpus's size, marked `xfail(strict=True)` with
the full reason in the test's own docstring and reason string (see
"Roadmap" in README.md and `LOGBOOK_09032026_152832.md` for the
investigation). Without Memgraph/PostgreSQL/the LLM model (e.g. this
repo's dev laptop, which additionally can't install `llama-cpp-python` at
all — see "Running the optional small-LLM answer layer (P7)"): **150
passed, 1 xfailed, 53 skipped** (29 `tests/kg/` + 21 `tests/access/` + 2
`tests/answer/` cases + the PDF test) — see "Testing against Memgraph" /
"Testing against PostgreSQL" above. `tests/crawl/` always runs regardless (offline, against
...
- `tests/eval/test_hybrid_vs_bm25.py` — the P2 exit criterion: builds both
  ...
  **Currently `xfail(strict=True)`** — genuinely not met at the current
  47-doc corpus size (+0.4%, see README.md's Roadmap section and
  `LOGBOOK_09032026_152832.md`), disclosed rather than silently reverted
  or loosened.
...
4. `PYTHONPATH=src .venv/bin/python -m pytest -q` — should print `150
   passed, 1 xfailed, 53 skipped` without Memgraph/PostgreSQL/the P7 model
   running, or start those first (see "Running the knowledge graph pipeline
   (P4)", "Running the access control / Git connector (P5)", and "Running
   the optional small-LLM answer layer (P7)") for `202 passed, 1 xfailed,
   1 skipped`. The `xfailed` is the P2 exit-criterion test — expected, see
   "Running the full test suite" above. First run needs network once
   (BGE-Small model download, ~130MB); after that it's fully offline except
   the one PDF-parsing test, which stays gated/skipped by default (see
   "Network dependencies" above).
